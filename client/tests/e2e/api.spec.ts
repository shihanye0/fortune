...
test.describe('API 接口测试', () => {
  ...
  test('登录接口正常', async ({ request }) => {
    ...
    await request.post(`${apiBaseUrl}/api/v1/auth/register`, {
      data: {
        username: '登录测试',
        email,
        password: 'Test1234',
  ...
  test('获取运势需要认证', async ({ request }) => {
    const response = await request.get(`${apiBaseUrl}/api/v1/fortunes/today`)
    expect(response.status()).toBe(401)
  })

  test('获取占卜记录需要认证', async ({ request }) => {
    const response = await request.get(`${apiBaseUrl}/api/v1/divination/records`)
    expect(response.status()).toBe(401)
  })
})
